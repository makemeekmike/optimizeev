...
import { PenTool as Tool, Calendar, AlertTriangle, Clock, CheckCircle, Filter, Plus, Search, Settings, BarChart, Package, FileText, Wrench, ChevronRight } from 'lucide-react';
...
const MaintenanceView = () => {
  ...
  const [activeTab, setActiveTab] = useState('schedule'); // 'schedule' or 'workOrders'
  ...
  const handleTabChange = (tab: string) => {
    setActiveTab(tab);
    navigate(`/dashboard/maintenance${tab === 'workOrders' ? '/orders' : ''}`);
  };
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex space-x-4">
          ...
          <button className="flex items-center px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700">
            ...
            {activeTab === 'workOrders' ? 'Create Work Order' : 'Schedule Maintenance'}
          </button>
        </div>
      </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-xl shadow-sm p-6">
                ...
                <div className="space-y-3">
                  ...
                  <button 
                    className="w-full flex items-center justify-between p-3 bg-secondary-50 rounded-lg hover:bg-secondary-100"
                    onClick={() => handleTabChange('workOrders')}
                  >
                    <div className="flex items-center">
                      <Wrench className="h-5 w-5 text-primary-600 mr-2" />
                      <span className="text-secondary-900">View Work Orders</span>
                    </div>
                    <ChevronRight className="h-5 w-5 text-secondary-400" />
                  </button>
                  <button className="w-full flex items-center justify-between p-3 bg-secondary-50 rounded-lg hover:bg-secondary-100">
